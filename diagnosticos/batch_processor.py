...
class BatchProcessor:
    ...
    def process_batch(self, batch_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process each assessment type with students
            for assessment_type, count in counts.items():
                if count > 0:
                    logger.info(f"Processing {assessment_type} with {count} students")
            ...
            # Send emails if any assessments were processed
            if results['assessments_processed'] > 0:
                logger.info("Sending emails")
                email_success = self._send_emails()
                if not email_success:
                    results['errors'].append("Failed to send emails")
    ...
    def _process_assessment_type(self, assessment_type: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                '--download', '--process', '--analyze', '--reports', '--incremental'
            ]
    ...
    def _send_emails(self) -> bool:
        """
        Send emails using send_emails.py
        
        Returns:
            True if successful, False otherwise
        """
        try:
            logger.info("Executing send_emails.py")
            ...
            logger.error("Timeout sending emails")
            return False
        except Exception as e:

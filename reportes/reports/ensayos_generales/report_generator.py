...
class ReportGenerator:
    def __init__(self):
        ...
        self.templates_dir = "templates/ensayos_generales"
        self.analysis_file = "data/ensayos_generales/analysis/analysis.csv"
    ...
    def _add_assessment_tables(self, html_content: str, student_row: pd.Series, username: str) -> str:
        ...
        try:
            ...
            for assessment_type in assessment_types:
                # Load questions file
                questions_file = f"data/ensayos_generales/questions/{assessment_type}.csv"
                ...
                # Load processed file
                processed_file = f"data/ensayos_generales/processed/{assessment_type}.csv"
          ...
          <td style="border: 1px solid #000; padding: 8px; text-align: center; background-color: {cell_color};">{marked_answer}</td>
            ...
            for i in range(0, len(table_rows), rows_per_page):
                ...
                assessment_names = {
                    ...
                    'HYST': 'Historia',
                    'CIENB': 'Ciencias mencion biologia',
                    'CIENF': 'Ciencias mencion fisica',
                    'CIENQ': 'Ciencias mencion quimica',
                    'CIENT': 'Ciencias tecnico profesional'
                }

                assessment_name = assessment_names.get(assessment_type, assessment_type)
                page_title = f"Resultados {assessment_name} - Pagina {page_num}" if len(table_rows) > rows_per_page else f"Resultados {assessment_name}"

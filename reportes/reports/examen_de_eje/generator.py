"""Examen de Eje plugin.

Builds a per-student unit-level performance report from:
1) LearnWorlds responses (via ids.xlsx examen-de-eje mappings), and
...
class ExamenDeEjeGenerator(BaseReportGenerator):
    ...
    def _load_examen_de_eje_mapping(self) -> list[MappingRow]:
        ...
        if not rows:
            raise ValueError("No valid EXAMEN DE EJE rows found in ids.xlsx")
        ...
        for row in data_rows:
            ...
            parsed = _EDE_NAME_RE.match(normalized_name)
            if not parsed:
                continue
    ...
    def download(self) -> dict[str, pd.DataFrame]:
        mapping = self._load_examen_de_eje_mapping()
        downloaded: dict[str, pd.DataFrame] = {}
        for item in mapping:
            assessment_name = f"{item.assessment_type}_EXAMEN_DE_EJE_{item.assessment_number}"
    ...
    def analyze(self, download_result: dict[str, pd.DataFrame]) -> dict[tuple[str, str], ExamenPlan]:
        ...
        mapping_by_assessment_name = {
            f"{m.assessment_type}_EXAMEN_DE_EJE_{m.assessment_number}": m for m in mapping
        }
    ...
    def render(self, analysis_result: dict[tuple[str, str], ExamenPlan]) -> Path:
        ...
        for (_, email), plan in analysis_result.items():
            ...
            # Static structural markers that appear in body.html

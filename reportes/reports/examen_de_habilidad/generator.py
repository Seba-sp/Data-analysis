...
Builds a per-student reading-task performance report from:
...
State logic per tarea lectora:
  correct / total * 100 -> shown as percentage string (e.g. "75%")

Overall recommendation:
  total_correct / total_questions >= 80% -> "HD" (Habilidad dominada)
  otherwise                              -> "HPD" (Habilidad por dominar)
"""
...
class HabilidadPlan:
    ...
    def overall_percent(self) -> float:
        total = sum(t.total for t in self.tareas.values())
        correct = sum(t.correct for t in self.tareas.values())
        if total == 0:
            return 0.0
...
class ExamenDeHabilidadGenerator(BaseReportGenerator):
    ...
    def _load_examen_de_habilidad_mapping(self) -> list[MappingRow]:
        ...
        if has_headers:
            ...
            name_idx, id_idx, data_rows = 0, 1, rows
        ...
        for row in data_rows:
            ...
            if bank_name not in self._available_bank_names():
                logger.warning("Question bank not found: %s", bank_name)
                continue
    ...
    def download(self, assessment_name: str = "") -> dict[str, pd.DataFrame]:
        ...
        for item in mapping:
            key = f"{item.assessment_type}_EXAMEN_DE_HABILIDAD_{item.assessment_number}"
    ...
    def analyze(self, download_result: dict[str, pd.DataFrame]) -> dict[tuple[str, str], HabilidadPlan]:
        ...
        for key, df in download_result.items():
            ...
            if "habilidad" in bank_df.columns:
                first_val = bank_df["habilidad"].dropna()
                if not first_val.empty:
                    habilidad_name = _display_text(first_val.iloc[0])

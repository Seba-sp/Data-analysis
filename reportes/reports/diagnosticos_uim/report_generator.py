...
class ReportGenerator:
    def __init__(self):
        ...
        self.templates_dir = "templates/diagnosticos_uim"
        self.questions_dir = "data/diagnosticos_uim/questions"
        self.processed_dir = "data/diagnosticos_uim/processed"
        self.analysis_dir = "data/diagnosticos_uim/analysis"
        self.lectures_file = "data/diagnosticos_uim/questions/lecciones.xlsx"
        ...
        <tr>
          ...
          {lecture_cell}
            ...
            for page_idx, page_rows in enumerate(table_pages):
                ...
                page_title = f"Resultados {assessment_name} - Pagina {page_num}" if len(table_pages) > 1 else f"Resultados {assessment_name}"
    ...
    <table style="width: 550px; border-collapse: collapse; margin: 20px auto; border: 1px solid #000; table-layout: fixed;">
      <thead>
        <tr>
          ...
          <th style="border: 1px solid #000; padding: 12px; text-align: center; background-color: #f0f0f0; font-weight: bold; width: 300px;">Leccion</th>
        </tr>
      </thead>
      <tbody>
        {''.join(page_rows)}

# Promoted from diagnosticos/complete_deployment/storage.py — Phase 2
import os
import pandas as pd
import json
...
class StorageClient:
    def __init__(self):
        ...
        if self.backend == 'gcs':
            if not self.bucket_name:
                raise ValueError("GCP_BUCKET_NAME environment variable is required when STORAGE_BACKEND=gcs")
    ...
    def get_backend_info(self):
        ...
        info = {
            ...
            'bucket_name': self.bucket_name if self.backend == 'gcs' else None
        }
        return info

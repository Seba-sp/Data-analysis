...
class AssessmentAnalyzer:
    def __init__(self, config: Dict[str, Any] = None):
        ...
        Args:
            config: Configuration dictionary for assessment types and parameters.
                    Pass None to use the default config (M1/CL/CIEN/HYST).
                    Other report types (e.g. diagnosticos_uim) pass their own config dict.
        """
        # Default configuration
        self.config = config or self._get_default_config()

    def _get_default_config(self) -> Dict[str, Any]:
        """Get default configuration for all assessment types (M1, CL, CIEN, HYST).

        Note: diagnosticos_uim assessment types (F30M, B30M, Q30M) are NOT included here.
        Those belong in the uim report module, which passes its own config at instantiation.
        """
        return {
            "assessment_types": {
                "M1": {
    ...
    def analyze_assessment(self, user_response: Dict[str, Any], question_bank: pd.DataFrame, assessment_name: str) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If assessment_name is not found in the config.
        """
        try:
    ...
    def _get_internal_level(self, reported_level: str, assessment_name: str, result: Dict[str, Any]) -> str:
        ...
        Returns:
            Internal level string (including "Nivel 4" when applicable for M1/CL)
        """
        try:
            assessment_config = self.config["assessment_types"][assessment_name]
            ...
            if assessment_name in ["M1"]:
                ...
                # For any other assessment type passed by other report modules (e.g. uim):
                # return the reported level — uim assessments have no Nivel 4 distinction
                return reported_level

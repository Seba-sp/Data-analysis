# Promoted from shared/slack_service.py — Phase 2
#!/usr/bin/env python3
"""
        ...
        try:
            ...
            blocks = [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": f"Reporte de Analisis de Curso - {course_name} ({category})"
                    }
                },
                {
                    "type": "section",
            ...
            blocks.append({
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": "Automatizado por el Pipeline de Analisis de Cursos"
                    }
                ]
            })

            # Create fallback text
            fallback_text = f"Reporte de Analisis de Curso - {course_name} ({category})\nCategoría: {category}\nCurso: {course_name}\nID del Curso: {course_id}\nAnálisis completado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            if drive_links:

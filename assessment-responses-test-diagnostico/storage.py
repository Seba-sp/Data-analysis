...
class StorageClient:
    ...
    def write_excel(self, path, df, **kwargs):
        if self.backend == 'local':
            df.to_excel(self._local_path(path), **kwargs)
        else:
            blob = self.bucket.blob(self._gcs_path(path))
            # For Excel files, we need to use BytesIO instead of StringIO
            from io import BytesIO
            excel_buffer = BytesIO()
            df.to_excel(excel_buffer, **kwargs)
            excel_buffer.seek(0)
